...
public class OrderServiceTest {
	...
	public void ordersListNasnavAdminDifferentFiltersTest() {
		...
		// by store_id
		response = template.exchange("/order/list?shop_id=501", HttpMethod.GET, new HttpEntity<>(header), String.class);
		body = new JSONArray(response.getBody());
		count = body.length();
		...
		response = template.exchange("/order/list?org_id=99001&shop_id=503", HttpMethod.GET, new HttpEntity<>(header), String.class);
		body = new JSONArray(response.getBody());
		count = body.length();
		...
		response = template.exchange("/order/list?shop_id=501&status=NEW", HttpMethod.GET, new HttpEntity<>(header), String.class);
		body = new JSONArray(response.getBody());
		count = body.length();
		...
		response = template.exchange("/order/list?user_id=88&shop_id=501&status=NEW", HttpMethod.GET, new HttpEntity<>(header), String.class);
		body = new JSONArray(response.getBody());
		count = body.length();
