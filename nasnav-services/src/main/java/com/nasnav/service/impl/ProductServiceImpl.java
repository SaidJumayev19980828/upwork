...
import com.nasnav.commons.utils.*;
import com.nasnav.constatnts.EntityConstants.Operation;
import com.nasnav.dao.*;
import com.nasnav.dto.*;
import com.nasnav.dto.request.product.*;
...
public class ProductServiceImpl implements ProductService {
	...
	@Override
	public List<ProductDetailsDTO> getProducts(List<Long> ids) throws BusinessException {
		List<ProductDetailsDTO> result = new ArrayList<>();
		if (ids == null || ids.isEmpty())
			return result;
		List<ProductEntity> products = productRepository.findAllByIdIn(ids);
		for (ProductEntity product : products) {
			List<ProductVariantsEntity> productVariants = getProductVariants(product, false);
			result.add(createProductDetailsDTO(product, null, productVariants, true));
		}
		return result;
	}
