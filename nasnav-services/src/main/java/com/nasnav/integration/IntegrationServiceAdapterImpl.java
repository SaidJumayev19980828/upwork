...
public class IntegrationServiceAdapterImpl implements IntegrationServiceAdapter {
	...
	public void pushCustomerCreationEvent(CustomerData customer, Long orgId) {
		var event =
				Event.ofType(CustomerCreateEvent.class)
					.setEventInfo(orgId, customer)
					.subscribeToResult(this::saveUserExternalId, this::generalIntegrationErrorHandler)
					.build();
		pushEvent(event);
	}
	...
	private Mono<EventResult<OrderData, OrderData.Result>> pushSubOrderFinalizeEvent(OrdersEntity order) {
		...
		OrderConfirmEvent event =
				Event.ofType(OrderConfirmEvent.class)
						.setEventInfo(orderData.getOrganizationId(), orderData)
						.subscribeToResult(this::saveOrderExternalId, this::generalIntegrationErrorHandler)
						.build();
...
//	}
	...
	public Mono<List<EventResult<PaymentData, String>>> pushPaymentEvent(PaymentAndOrderIdMapping payment, MetaOrderEntity metaOrder){
		...
		return createPaymentPerSubOrder(payment, metaOrder)
				.map(subPayment -> createPaymentCreateEvent(orgId, subPayment))
	...
	private PaymentCreateEvent createPaymentCreateEvent(Long orgId, PaymentData subPayment) {
		return Event.ofType(PaymentCreateEvent.class)
					.setEventInfo(orgId, subPayment)
					.subscribeToResult(FunctionalUtils::doNothing)
					.build();
	}
